...
**The `/work-hours/` nav link carries a "new sign-ups" badge (issue
#217).** A "new work-session sign-ups" count
(`count_new_participations()`, `app/services/work_hours.py`) tracking
recently-created `SessionParticipation` rows -- whether created through
the public self-service signup form or manually by a board member on a
session's detail page -- with a matching dashboard tile and email, plus
a "New" badge per row on the session detail page's participant table
(`session_detail.html`). See
[ADR 0079](./ADR/0079-work-session-signup-notifications-stateless-not-a-notification-center.md)
